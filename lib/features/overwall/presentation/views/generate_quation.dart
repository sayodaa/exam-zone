...
class _GenerateExamScreenState extends State<GenerateExamScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<GenerateExamCubit, GenerateExamState>(
        listener: (context, state) {
          if (state is GeneratingExamInProgress) {
            setState(() {
          ...
          } else if (state is GenerateExamFailure) {
            setState(() {
              _isGenerating = false;
              generatingProgress = 0.0;
            });
  ...
  void _startGenerating() async {
    if (_formKey.currentState!.validate()) {
      ...
      final cubit = context.read<GenerateExamCubit>();
      await cubit.generateExam(
